...
export const PATCH =async (
  ...
  try {
    ...
    if (!product) return NextResponse.json({ error: 'Invalid Issue' }, { status: 404 });
    ...
    if (data.get('file') !== product.image) {
      ...
      where: { id: product.id },
      data: {
        image: downloadUrl, name, description, price: parseInt(price), category, type
      }
    });
    } else {
      await prisma?.product.update({
        where: { id: product.id },
        data: {
          image: product.image, name, description, price: parseInt(price), category, type
        }
      });
    }
